...
\item Let's show that for any integer $n>1$ the Laplace Transform of $f(t)=t^n$ satisfies $\displaystyle \mathcal{L}(t^n)=\frac{n+1}{s}\mathcal{L}(t^{n-1})$ for $s>0$, by applying the definition, $\mathcal{L}(f)=\displaystyle \int_0^{\infty}f(t)e^{-st}\,dt$. 
...
\begin{enumerate}
\item First let's fully-factor the denominator and perform a PFD for $F(s)$.
